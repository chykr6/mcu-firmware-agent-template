...
scripts/               通用 MDK/clangd 辅助脚本及其测试，不放产品业务脚本
...
- `scripts/mdk_build.py`：读取 `toolchain.local.json`，发现 `.uvprojx` 和 target，并调用 uVision 构建。
- `scripts/mdk_clangd.py`：读取 MDK 工程，在仓库根目录生成通用的 `compile_commands.json` 和 `.clangd`。
- `scripts/mdk_flash.py`：使用 target 已有下载配置烧录固件。
- `scripts/mdk_clean.py`：按 MDK 工程输出配置清理构建产物。
- `scripts/mdk_common.py`：公共配置、路径和命令执行工具，不直接运行。
- `scripts/tests/`：Python 标准库单元测试，不承载 MCU 业务测试。

`toolchain.local.json` 是本机文件，不提交。存在多个 MDK 工程或 target 时，使用
`--project` 和 `--target` 显式选择；脚本不会把 `toolchain.example.json` 当作本机配置执行。
